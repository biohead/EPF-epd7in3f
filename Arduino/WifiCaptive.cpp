...
bool WifiCaptive::startPortal()
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (res)
            {
                saveWifiCredentials(_ssid, _password, _api_server);
...
std::vector<WifiCaptive::Network> WifiCaptive::getScannedUniqueNetworks(bool runScan)
{
    ...
    for (auto &network : uniqueNetworks)
    {
        Serial.printf("SSID: %s, RSSI: %d, Open: %d\n", network.ssid.c_str(), network.rssi, network.open);
...
bool WifiCaptive::autoConnect()
{
    ...
    if (_savedWifis[last_used_index].ssid != "")
    {
        ...
        Serial.println("Trying to connect to last used ssid.");
        WiFi.setSleep(0);
    ...
    std::vector<Network> scanResults = getScannedUniqueNetworks(false);
